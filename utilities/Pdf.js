...
var PDFJS = require('pdfjs-dist'); // "pdfjs-dist": "^2.16.105",
Image = function () { };
...
global.btoa = function (input = '') {
    ...
    for (let i = 0; i < str.length; i += 3) {
        ...
        const b2 = str.charCodeAt(i + 1) & 0xFF;
        const b3 = str.charCodeAt(i + 2) & 0xFF;
        const d1 = b1 >> 2;
        const d2 = ((b1 & 3) << 4) | (b2 >> 4);
        const d3 = i + 1 < str.length ? ((b2 & 0xF) << 2) | (b3 >> 6) : 64;
        const d4 = i + 2 < str.length ? (b3 & 0x3F) : 64;
        output += chars[d1] + chars[d2] + chars[d3] + chars[d4];
    }
    return output;
};
...
DOMElement.prototype = {
    ...
    toString: function () {
        const attrList = Object.entries(this.attributes)
            .map(([k, v]) => `${k}="${xmlEncode(v)}"`);

        if (this.nodeName === 'svg:tspan' || this.nodeName === 'svg:style') {
            return `<${this.nodeName} ${attrList.join(' ')}>${xmlEncode(this.textContent)}</${this.nodeName}>`;
        } else if (this.nodeName === 'svg:svg') {
            const ns = 'xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svg="http://www.w3.org/2000/svg"';
...
pdf_table_extractor = function (doc) {
    ...
    // Matrix multiplication for 2D affine transforms (PDF.js style)
    function transform_fn(m1, m2) {
        return [
            m1[0] * m2[0] + m1[2] * m2[1],
            m1[1] * m2[0] + m1[3] * m2[1],
            m1[0] * m2[2] + m1[2] * m2[3],
    ...
    // Applies a 2D affine transform to a point [x, y]
    function applyTransform_fn([x, y], m) {
        return [
            x * m[0] + y * m[2] + m[4],
            x * m[1] + y * m[3] + m[5]
        ];
...
pdf_table_extractor_run = function (pdfPath, success, error) {
    ...
    const loadingTask = PDFJS.getDocument({
        data: data,
        standardFontDataUrl: standardFontsPath + path.sep,
